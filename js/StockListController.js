var screenerApp = angular.module('Screener', []);

screenerApp.controller('StockListController', function ($scope, $http) {

    function withRawTransform(transform) {
    ...
    function nextExpireDate() {
        var today = new Date();
        var nextExp = expireSaturday(today);
        if (nextExp <= today) {
            nextExp = expireSaturday(new Date(today.getYear() + 1900, today.getMonth() + 1, 1))
        }
        return nextExp;
    }
    ...
    function downloadOptions(stock) {
        ...
        var expD = exp.getDate();
        var optUrl = "https://www.google.com/finance/option_chain"; //?q="+stock.symbol+"&expd="+expD+"&expm="+expM+"&expy="+expY+"&output=json";

        $http({url: optUrl,
            method: "GET",
            params: {q: stock.symbol, expd: expD, expm: expM, expy: expY, output: "json"},
            transformResponse: withRawTransform(function(rawVal) {
                return rawVal.replace(/(['"])?([a-zA-Z0-9_]+)(['"])?:/g, '"$2": ');
            })
    ...
    function downloadStocks() {
        var list = _.map($scope.stocks, function(stock) { return '"' + stock.symbol + '"' }).join(",");
        var query = "select * from yahoo.finance.quote where symbol in ("+list+")";

        function enrich(quotes) {
            _.each($scope.stocks, function(s) {
                s.quote = _.find(quotes, function(q) { return s.symbol == q.symbol });
            });
        }
        ...
        ).success(function(res) { enrich(res.query.results.quote); })

    }

    $scope.currentStock = null;
    $scope.showStock = function(stock) {
        $scope.currentStock = stock;
        if (angular.isUndefined(stock.chains)) downloadOptions(stock);
    };
